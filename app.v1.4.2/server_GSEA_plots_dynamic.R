observeEvent(input[[paste0("choose_GO_", gsea_cond )]], {
  ...
  if( input[[paste0("choose_GO_", gsea_cond )]] != "" && length(input[[paste0("choose_GO_", gsea_cond )]]) > 0 && !is.na(input[[paste0("choose_GO_", gsea_cond )]])){  
    ...
    plot_data = as.data.frame(rv$gsea_list[[gsea_cond]])
    geneset =  which(plot_data$Description == input[[paste0("choose_GO_", gsea_cond )]])
    ...
    if(length(geneset) >0 ){
      ...
      rv[[paste0("gseaplot","_", gsea_cond)]] = empty_image()
    }
