...
observeEvent(input$selectall_galaxy, {
  if(input$selectall_galaxy > 0 ){
    ...
    if(input$selectall_galaxy %% 2 == 1 ){
      updateSelectInput(session, "galaxy_outputs", choices = rv$galaxy_outputs$file, selected = rv$galaxy_outputs$file)
      rv$aws_files <- rv$galaxy_outputs$file
    }else{
      updateSelectInput(session, "galaxy_outputs", choices = rv$galaxy_outputs$file, selected = c())
      rv$aws_files <- c()
    }
...
observeEvent(input$submit_contrast, {
  ...
  print(paste(colnames(filtered), collapse = ","))
  x <- group
  create_msi_df = F
  ...
  hideTab(inputId = "norm_de_plot_tabsetpanel", target = "Volcano plot")
  hideTab(inputId = "norm_de_plot_tabsetpanel", target = "Heat-Map plot (pairwise)")
  hideTab(inputId = "norm_de_plot_tabsetpanel", target = "Heat-Map plot (overview)")
