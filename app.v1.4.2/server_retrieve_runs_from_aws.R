...
observeEvent(input$submit_username, {
  ...
  if(length(l) > 0){
    ...
    showNotification(paste("No User with that username"), duration = 0, type = "error")
  }
...
observeEvent(input$submit_galaxy_outputs, {
  ...
  rv$metadata_df <- get_manual_metadata() %>%
    filter(names %in% input$galaxy_outputs)
  print("METADATA")
  print(rv$metadata_df )

  # rv$galaxy_outputs = data.frame(file = c("SRR1207056.tabular","SRR7637208.tabular" , "SRR7637249.tabular" , "SRR7637250.tabular", "SRR7637252.tabular", "SRR7637253.tabular"),
  #                                path = c( "tmp/SRR1207056.tabular", "tmp/SRR7637208.tabular", "tmp/SRR7637249.tabular" , "tmp/SRR7637250.tabular","tmp/SRR7637252.tabular","tmp/SRR7637253.tabular"))
  # 
