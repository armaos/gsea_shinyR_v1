...
####
for(cond in input$choose_conditions){
  ...
  rv[[paste0("volcano0","_",cond)]] <- ggplot(data=v0, aes(x=logFC, y=-log10(FDR), color=change, fill=change), fontface='bold') +
  ...
  rv[[paste0("pheatmap","_",cond)]] <- pheatmap(id2,fontsize=8, angle_col=0, cutree_rows=2, cutree_col=2, scale="row", cluster_cols=F, treeheight_col=5, treeheight_row=15, color = colorRampPalette(c("navy", "white", "firebrick3"))(100), annotation=annotation, width=6, height=12, show_rownames=F)
  ...
  for(col in c("logCPM", "logFC", "LR", "PValue", "PValue", "FDR", "genelabels", "change")){
    colnames(v0) = rename_msi_df_columns(colnames(v0), col, paste(cond, col, sep = "_"))
  }
