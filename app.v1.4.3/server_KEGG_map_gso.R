  ...
  if(rv$okplot_KEGGmap_gso){
    ...
    if( rv$map != "" && length(rv$map) > 0 && !is.na(rv$map )){  
      ...
      if (input$kegg_gso_plotting_radio == "annotations"){
        ...
        map_legend = c("KFK Koala", "eggNOG mapper")
        
        max_range = 1
        min_range = 0
      }else if (input$kegg_gso_plotting_radio == "conditions"){
        map_legend = colnames(kegg_gso_vector  %>% select(contains("logFC")))
      ...
      print(outfile)
      html("legend_KEGG_map_gso", paste0("<b>LEGEND</b><br>Annotations of nodes are displayed with the following order:<br>",
                                         paste(map_legend, collapse = " | ")))
