...
withProgress(message = 'FastQC reports', value = 0, {
  incProgress(0.1, detail = paste("Identify FasQC reports..."))
  ...
  fastqc_files <<- data.frame(files = list.files("fastqc", pattern = "fastqc.zip$", full.names = TRUE)) %>%
    ...
    filter(Runacc %in% clean_RunAcc(fastqc_runs)) 
    
  incProgress(0.2, detail = paste("Reading FastQC reports..."))
  ...
  if(length(fastqc_files$files) > 0)
  {
    ...
    showNotification(paste("No available FastQC data"), duration = 5, type = "error")
    rv$show_menu_fastqc = FALSE
  }
  incProgress(0.9, detail = paste("Preparring FastQC plots..."))
})
