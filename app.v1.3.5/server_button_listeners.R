...
observeEvent(
  eventExpr = input$submit_thresholds, {
    ...
    print(paste("filtered genes load succesfully by thresholds", length(rv$input_data)[1]))
  })
...
observeEvent(
  eventExpr = input$submit_gene_select_by_group, {
    ...
    rv$fc = msi_df$logFC
    names(rv$fc)= rv$input_data
    print(paste("filtered genes load succesfully by group", length(rv$input_data)[1]))
  })
...
observeEvent(
  eventExpr = input$submit_custom_gene_set, {
    ...
    print(paste("custom genes load succesfully by input names", length(rv$input_data)))
  })
  ...
  eventExpr = input$run_gsea, {
    if (rv$condition == ""){
      ...
      print("GSEA")
        ...
        rv[[paste0("dotplot_gsea","_",id)]] = dotplot(rv$gsea)
        
        print("doing ridgeplot_gsea")
        #rv$ridgeplot_gsea = ridgeplot(rv$gsea )
        rv[[paste0("ridgeplot_gsea","_",id)]] = ridgeplot(rv$gsea )
    ...
    for(id in rv$condition){
      ...
      if(length(rv$ora ) > 0){
        ...
        if(length(rv$ora_desc) > 0 ){
          ...
          rv[[paste0("barplot1","_",id)]] = barplot(rv$ora, showCategory=rv$ora_desc, main = "Barplot (count)" )
          
          print("barplot2")
          # rv$barplot2 = mutate(rv$ora,
          #                      qscore = -log(p.adjust, base=10)) %>%
          #   barplot(x="qscore", showCategory=rv$ora_desc, main ="Barplot (qscore)")
          rv[[paste0("barplot2","_",id)]] = mutate(rv$ora,
                               qscore = -log(p.adjust, base=10)) %>%
            barplot(x="qscore", showCategory=rv$ora_desc, main ="Barplot (qscore)")
          
          print("dotplot")
          #rv$dotplot = dotplot(rv$ora, showCategory=rv$ora_desc)
          rv[[paste0("barplot1","_",id)]] = dotplot(rv$ora, showCategory=rv$ora_desc)
          ...
          rv[[paste0("emapplot","_",id)]] = emapplot(p_t, showCategory = rv$ora_desc)
          
          print("cnetplot")
          #rv$cnetplot = cnetplot(rv$ora, categorySize="pvalue", showCategory = rv$ora_desc, foldChange = rv$fc )
          rv[[paste0("cnetplot","_",id)]] = cnetplot(rv$ora, categorySize="pvalue", showCategory = rv$ora_desc, foldChange = rv$fc )          
          
          print("heatplot")
          #rv$heatplot = heatplot(rv$ora, showCategory = rv$ora_desc, foldChange = rv$fc )
          rv[[paste0("heatplot","_",id)]] = heatplot(rv$ora, showCategory = rv$ora_desc, foldChange = rv$fc )
          
          print("upsetplot")
          #rv$upsetplot = upsetplot(rv$ora, showCategory = rv$ora_desc)
          rv[[paste0("upsetplot","_",id)]] = upsetplot(rv$ora, showCategory = rv$ora_desc)
          
          print("treeplot")
          #rv$treeplot = treeplot(p_t)
          rv[[paste0("treeplot","_",id)]] = treeplot(p_t)
