  ...
  observeEvent(
    rv$update_msi_df, {
      ...
      default_columns = default_columns[default_columns != "primary_name"]
  ...
  observeEvent(input$choose_GO, {
    ...
    if( input$choose_GO != "" && length(input$choose_GO) > 0 && !is.na(input$choose_GO)){  
      ...
      for(id in rv$condition){
        rv[[paste0("gseaplot","_",id)]] = gseaplot2(rv$gsea, geneSetID = which(rv$gsea$Description == input$choose_GO) , title = input$choose_GO)
  ...
  observeEvent(
    eventExpr = input$choose_gso, {
      if(input$choose_gso == 'By group'){
        ...
        updatePickerInput(session, "gene_select_by_group", choices = choices_by_group_selection, selected = NULL)
      }
    })
  ...
  observeEvent(
    eventExpr = input$gene_select_by_group, {
      if(input$choose_gso == 'By group'){
        values = rv$msi_df %>% 
          select(all_of(input$gene_select_by_group)) %>% 
          filter(!is.na(!!as.symbol(input$gene_select_by_group)) &  !!as.symbol(input$gene_select_by_group) != "-") %>%
          separate_rows(!!as.symbol(input$gene_select_by_group), sep = "[,]")
